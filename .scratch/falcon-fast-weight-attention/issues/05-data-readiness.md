# Data readiness for local science and hero runs
...
- Inventory usable FineWeb-Edu and/or C4 slices already on this host
  (reuse `experiments/qwen3_fineweb_hsdp_tp/` or other repo loaders).
- Measure free disk vs corpus + checkpoint headroom. Fail loud if the
  slice will not fit.
- Recommend concrete Step 4 and Step 5 token budgets, seq_len, and
  global batch that keep tokens/step matched across Falcon / GDN-style /
  Transformer controls.
- Do not download a 50B corpus. Campaign B is local-hero scale.

## Exclusions

- Training runs
- Mixer or model code
- Paper-scale 49.2B authorization

## Verification

- A short readiness note (path, token count, disk, proposed Step 4/5
  knobs) appended under `## Answer`.
