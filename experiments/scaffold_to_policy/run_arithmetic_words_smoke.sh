...
for split in dev ood_test; do
  python -m torchtitan.experiments.scaffold_to_policy.cli write-arithmetic-fixture \
    --problems "${DATA_ROOT}/${split}.jsonl" \
    --output "${RESULTS_ROOT}/eval/${split}_fixture_rollouts.jsonl"

  python -m torchtitan.experiments.scaffold_to_policy.cli evaluate-arithmetic-fixture \
    --problems "${DATA_ROOT}/${split}.jsonl" \
