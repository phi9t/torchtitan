...
# Wave F4 public reasoning: MATH (hendrycks_math) public-dataset vLLM smoke
# driven through the typed begin/stage/finish lifecycle and a composed
...
if [[ "${TORCHTITAN_IN_ROOTFS:-0}" != "1" ]]; then
  exec "${REPO_ROOT}/scripts/rootfs/enter_rootfs.sh" -- "experiments/scaffold_to_policy/run_math_public_vllm_smoke.sh" "$@"
fi
...
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)-math-public-vllm-smoke}"
ATTEMPT_ID="${ATTEMPT_ID:-attempt-01}"
MODEL="${MODEL:-./assets/hf/Qwen3-1.7B}"
DATASET="${DATASET:-EleutherAI/hendrycks_math}"
DATASET_SUBSET="${DATASET_SUBSET:-algebra}"
DATASET_REVISION="${DATASET_REVISION:-21a5633873b6a120296cce3e2df9d5550074f4a3}"
SOURCE_SPLIT="${SOURCE_SPLIT:-test}"
DATA_ROOT="${DATA_ROOT:-experiments/scaffold_to_policy/data/math_public_vllm_smoke}"
RESULTS_ROOT="${RESULTS_ROOT:-experiments/scaffold_to_policy/results/math_public_vllm_smoke}"
...
run_stage validate-splits verify rootfs_cpu \
  ${CLI} validate-math-style-splits \
  --split \
...
run_stage build-report-input report rootfs_cpu \
  ${CLI} build-math-style-report-input \
  --data-root "${DATA_ROOT}" \
