...
if [[ "${TORCHTITAN_IN_ROOTFS:-0}" != "1" ]]; then
  exec "${REPO_ROOT}/scripts/rootfs/enter_rootfs.sh" -- "experiments/scaffold_to_policy/run_modular_sequences_smoke.sh" "$@"
fi

cd "${REPO_ROOT}"
export PYTHONPATH="${REPO_ROOT}:${PYTHONPATH:-}"

RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)-modular-sequences-smoke}"
DATA_ROOT="${DATA_ROOT:-experiments/scaffold_to_policy/data/modular_sequences_smoke}"
RESULTS_ROOT="${RESULTS_ROOT:-experiments/scaffold_to_policy/results/modular_sequences_smoke}"
...
for split in dev ood_test; do
  python -m torchtitan.experiments.scaffold_to_policy.cli write-modular-fixture \
    --problems "${DATA_ROOT}/${split}.jsonl" \
    --output "${RESULTS_ROOT}/eval/${split}_fixture_rollouts.jsonl"

  python -m torchtitan.experiments.scaffold_to_policy.cli evaluate-modular-fixture \
    --problems "${DATA_ROOT}/${split}.jsonl" \
