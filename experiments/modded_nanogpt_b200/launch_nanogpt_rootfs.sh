...
SCRIPT_REL="experiments/modded_nanogpt_b200/launch_nanogpt_rootfs.sh"

if [[ "${TORCHTITAN_IN_ROOTFS:-0}" == "1" && -n "${MODDED_NANOGPT_ROOTFS_RUN_ID:-}" ]]; then
  RUN_ID="${MODDED_NANOGPT_ROOTFS_RUN_ID}"
else
  RUN_ID="nanogpt_rootfs_smoke_$(date -u +%Y%m%dT%H%M%SZ)"
  export MODDED_NANOGPT_ROOTFS_RUN_ID="${RUN_ID}"
fi
...
if [[ "${TORCHTITAN_IN_ROOTFS:-0}" != "1" ]]; then
  ...
  TORCHTITAN_ROOTFS_EMIT_PLAN_ONLY=1 \
    ...
    "${REPO_ROOT}/scripts/rootfs/enter_rootfs.sh" -- \
      /bin/bash -lc "MODDED_NANOGPT_ROOTFS_RUN_ID='${RUN_ID}' exec '${SCRIPT_REL}'"
...
log "training: starting guarded Lane B smoke run through run_speedrun.sh"
set +e
